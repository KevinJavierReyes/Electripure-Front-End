...
function InputSelect({ state, message, name, options, placeholder, label, onChange } : {
    ...
    return (
        <div className="w-full styled-select">
            <label htmlFor={name}
                className={
                    "f-medium " + (state === STATE_INPUT_CONTROL.OK ? "color-success" : state === STATE_INPUT_CONTROL.ERROR ? "color-error" : "color-black-dark")}>
                {label}</label>
            <div className="h-[50px] w-full relative mt-[5px]">
                ...
                <select onChange={handleChange}
                    ...
                    className={"bg-transparent absolute top-0 left-0 m-0 w-full border h-[50px] px-[10px] " + (state === STATE_INPUT_CONTROL.OK ? "border-color-success color-success" : state === STATE_INPUT_CONTROL.ERROR ? "border-color-error color-error" : "border-color-black-light color-black")}
                    id={name}>
