...
function DataTableUsers({}) {
    ...
    async function deactivateUser(user: UserEntity) {
        dispatch(sendActivateDeactivateUser({"id": user.id, "action": "deactivate"}))
    }
    
    async function activateUser(user: UserEntity) {
        dispatch(sendActivateDeactivateUser({"id": user.id, "action": "activate"}))
    }
        ...
        return {
            ...
            "Status": {
                "label": user.Status == "Active" ? 
                        <span>
                            <span className="color-success f-bold">{user.Status}</span>
                            <span className="cursor-pointer color-secondary underline f-light text-sm ml-[10px]" onClick={()=>{deactivateUser(user)}}>deactivate</span>
                        </span> :
                        user.Status == "Desactivate" ? 
                        <span>
                            <span className="color-secondary f-bold">{user.Status}</span>
                            <span className="cursor-pointer color-secondary underline f-light text-sm ml-[10px]" onClick={()=>{activateUser(user)}}>activate</span>
                        </span>:
                        <span>
                            <span className="color-error f-bold">{user.Status}</span>
                            <span className="cursor-pointer color-secondary underline f-light text-sm ml-[10px]" onClick={()=>{resendInvite(user)}}>resend invite</span>
                        </span>,
                "value": user.Status
            },
            "Date": {
