...
import {
  sendGetCompaniesTable,
  sendUpdateCompany,
  sendGetCompanyDetail,
  sendCreateMDP,
  sendCreateSite,
  sendGetCompaniesByUser,
  sendArchiveCompany,
...
const CompanyDetails = () => {
  ...
  const companies = JSON.parse(
    useSelector((state: ElectripureState) => state.companiesTable)
  );
  const isActivateDefault =
    (companies && companies.length > 0
      ? companies.find((company: any) => company.id === parseInt(ciaId ?? ""))
      : {
          status: "Active",
        }
    ).status === "Active";
  const [isActivate, setIsActivate] = useState(isActivateDefault);
  const company: ICompany = JSON.parse(
    useSelector((state: ElectripureState) => state.companyDetails)
  );
  // Company detail
  //   company.contact_name = "Kevin Javier";
  //   company.contact_email = "kevin@outcode.com";
  //   company.contact_phone = "924017549";
  //   company.sites[0].contact_name = "Kevin Javier";
  //   company.sites[0].contact_email = "kevin@outcode.com";
  //   company.sites[0].contact_phone = "924017549";
  //   company.sites[0].mdps[0].location = {
  //     x: 20,
  //     y: 20,
  //   };
  ...
  async function deactivateCompany() {
    dispatch(
      sendActivateDeactivateCompany({ id: companyId, action: "deactivate" })
    );
  }
  async function activateCompany() {
    dispatch(
      sendActivateDeactivateCompany({ id: companyId, action: "activate" })
    );
  }
  // !SECTION
  // SECTION
      ...
      <div className="bg-white border w-full h-full p-[40px] overflow-auto">
        ...
        <div className="flex justify-between items-start w-[100%]">
          ...
          <div className=" flex flex-col justify-center items-center">
            {/* <div
              className="cursor-pointer flex items-center relative w-[100px]"
        ...
        <div className="flex justify-center items-center my-[20px]">
          ...
          <CiaPermission role="edit_company">
            <button
              ...
              onClick={() => setToggleModalCreateSite(!toggleModalCreateSite)}
            >
              + Add New Site
            </button>
          </CiaPermission>
        ...
        <div>
          {company?.sites
            ? company?.sites.map((site: ISite, index: number) => (
                <SiteDetails key={index} site={site} />
              ))
            : ""}
        </div>
        {/* !SECTION */}
